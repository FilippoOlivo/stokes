...
Stokes<dim>::Stokes(unsigned int degree)
    ...
    : degree(degree)
    , fe(FE_Q<2>(degree + 1) ^ 2, FE_Q<2>(degree))
    , dof_handler(triangulation)
    , computing_timer(std::cout, TimerOutput::never, TimerOutput::wall_times){};

template <int dim>
void
Stokes<dim>::load_grid()
{
    GridIn<2> grid_in;
...
template <int dim>
void
Stokes<dim>::setup_system_matrix()
{
...
template <int dim>
void
Stokes<dim>::setup_preconditioner_matrix()
{
...
template <int dim>
void
Stokes<dim>::build_local_preconditioner_matrix(
    std::vector<double> &phi_p,
    double               JxW,
    const unsigned int   dofs_per_cell,
...
Stokes<dim>::assemble_system()
{
    ...
    A_preconditioner = SparseDirectUMFPACK();
...
template <int dim>
void
Stokes<dim>::output_results(unsigned int cycle)
{
