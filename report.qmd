...
To study the scalability of the code in a more detailed way we decided to split logically split in the following sections and compute the separate speedup for each of them:
...
- **Residual computation**: time taken to compute the residual at each Netwton iteration. 
...
- **Weak scalability**: For a fixed number of processors, we analyze how the computational time evolves as the problem size grows. In our case, the problem size is increased by adaptively refining the mesh, which increases the number of degrees of freedom.

We consider the speed-up for solving a the Navier-Stokes problem with $300000$ degrees of freedom and $5$ Newton iterations (this is just for scalability testing purposed).


We consider the speed-up achieved when solving the Navier–Stokes problem with $300{,}000$ degrees of freedom and $15$ Newton iterations. The figure below shows the speed-up values for different components of the algorithm as the number of processors increases. Overall, we observe a modest speed-up, reaching a maximum value of around $10$ when using $32$ processors. As expected, the components that benefit the most from parallelization are the assembly and residual computation. These parts involve relatively little inter-processor communication and are also the most computationally intensive in the serial version of the program. In contrast, the solving phase and Schur complement initialization exhibit limited speed-up. This is likely due to the significant communication and memory overhead associated with these steps, which limits their scalability.
