...
- Momentum conservation: This accounts for the effects of internal and external forces acting on the fluid. Internal forces include viscous stresses (from fluid friction) and pressure gradients, while external forces typically involve gravity. In Stokes equation the momentum conservation is expressed as: 
    ...
    \nu \Delta u + \nabla p = f
    $$
    where essentially the term $\nu \Delta u$ represent the linear viscous forces, while the term $\nabla p$ represent the forces caused by the pressure gradients. This first equation negletes the convective acceleration non linear terms which takes into account the internal friction caused by the velocity gradients, responsible of complex phenomena such as vortices and turbolence. This additional terms has been introduced in Navier-Stokes equation:
    $$
    (u \cdot \nabla)u - \nu \Delta u + \nabla p = f
    $$

In the context of fluid dynamics, a key quantity is the Reynolds number, a dimensionless value which gives an estimation of the ration between inertial and viscous forces in the flow. It is defined as:
$$
\text{Re} = \frac{U L}{\nu}
$$
...
![Navier Stokes streamine at different Reynolds number](img/ns_vortex.png)
...
In this section we discuss the computational requirements and scalability of Stokes and Navier Stoks models. All the tests have been preformed on a Rocky Linux 8 machine with $2\times$ Intel(R) Xeon(R) Gold $6258R$ CPU @ $2.70$ GHz  with $28$ cores each and $256$ GB of RAM.
