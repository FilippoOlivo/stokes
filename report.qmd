...
$$
\begin{cases}
-(\nu\Delta \delta \vec{u} + \nu \Delta \vec{u}_k) + (\vec{u_k}\cdot \nabla \delta \vec{u} + \vec{u_k}\cdot \nabla\vec{u}_k) + (\delta \vec{u} \cdot \nabla \vec{u}_k + {\color{red} \vec{u}_k\cdot \nabla\vec{u}_k}) + (\Delta \delta P + \Delta P_k) = \vec{f} + {\color{red} \vec{u}_k\cdot \nabla\vec{u}_k} \\
...
$$
\begin{cases}
-\nu \Delta \vec{u}_{k+1} + \vec{u}_k \cdot \nabla \vec{u}_{k+1} + \vec{u}_{k+1} \cdot \nabla \vec{u}_k + \nabla P_{k+1} = \vec{f} + {\color{red} \vec{u}_k\cdot \nabla\vec{u}_k} \\
\text{div}(\vec{u}_{k+1}) = 0
\end{cases}
$$

Similarly to what we did for stokes, let derive the weak formulation of the problem. At each iteration of the Newton algorithm we must solve the following linear system:
$$
\begin{bmatrix}
...
For the resolution of the linear system, both in the **Stokes** and **Navier–Stokes** problems, we use the **Schur complement** strategy.
...
The difficulty with the Schur complement arises in the Navier–Stokes problem, particularly at moderate Reynolds numbers, where the matrix $begin:math:text$ A $end:math:text$ becomes strongly non-symmetric due to the convection term.

To improve solver robustness, we adopt an Augmented Lagrangian formulation by adding a stabilization term to the velocity block (as proposed in tutorial step-57 of the deal.II library):
...
The problem we want to solve is a variant of the flow past cylinder problem which involves two different cylinders. The domain we consider is a simple $2D$ rectangular domain with shape $5\times 1.5$. The cylinders have radius of $0.1$ and their centers are places in $(0.5, 1)$ and $(1.5,0.5)$. The following image represent the mesh:

![Mesh](img/mesh.png){width=80%}

We impose no-slip boundary condition on $\Gamma_{\text{top}}, \Gamma_{\text{bottom}} \text{ and } \Gamma_{\text{out}}$ and on the surface of the cylinders. The inlet velocity, placed imposed on $\Gamma_{\text{in}}$ has the form:
$$
...
All simulations have been implemented using the deal.II finite element library. The code is structured in a modular and object-oriented manner, with the following main classes:
...
Moreover, we employed an adaptive refiniment strategy based on Kelly Estimator.
...
In this section we discuss the results of the simulations we have made on both Stokes and Navier-Stokes problem. Specifically, this chapter will be structures in the following way:

#. Stokes simulation
#. Navier-Stokes simulation
#. Comparison of Stokes and Navier-Stokes on low Reynolds
...
As mentioned in the previous section, we have also implemented a Stokes solver which employes non-stable FEM couple $P^k-P^k$ together with Brezzi-Pitkaranta stabilization. Recalling the section ..., the parameters we need to define is $\delta$. We compare the behaviour of the FEM solution at different values of the stabilization parameter $\gamma$

![$\gamma = 1$](img/bp_stokes_1.png){width="90%"}

![$\gamma = 0.1$](img/bp_stokes_0.1.png){width="90%"}

![$\gamma = 0.01$](img/bp_stokes_0.01.png){width="90%"}

From the three images above, we can see that a small $\gamma$ leads to a spurious pressure field, indicating a violation of the inf-sup condition due to insufficient stabilization. On the other hand, a large $\gamma$ results in a pressure field that is overly smooth and reduced in magnitude. In both cases, the velocity field remains mostly accurate.
